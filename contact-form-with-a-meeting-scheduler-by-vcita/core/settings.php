...
/**
 * Polyfill array_replace_recursive in case the user is on php < 5
 * @since 2.0.0
 */

if ( ! function_exists( 'array_replace_recursive' ) ) {
	
	function array_recurse_function( $array, $array1 ) {
		foreach ( $array1 as $key => $value ) {
			...
			if ( is_array( $value ) ) {
				$array[ $key ] = array_recurse_function( $array[ $key ], $value );
			}
			else {
				$array[ $key ] = $value;
			}
	...
	function array_replace_recursive( $array, $array1 ) {
		...
		for ( $i = 1; $i < count( $args ); $i ++ ) {
			if ( is_array( $args[ $i ] ) ) {
				$array = array_recurse_function( $array, $args[ $i ] );
			}
		}
